...
#[get("/check?<id>&<guess>")]
pub fn check(id: i64, guess: &str) -> Json<Resp<Vec<u8>>> {
    let res = db::get_problem_by_id(id);
    match res {
        ...
        Ok(problem) => {
            ...
            if let Err(e) = db::insert_guess(id, guess) {
                dbg!(e);
...
#[get("/getGuesses?<id>")]
pub fn get_guesses(id: i64) -> Json<Resp<Vec<String>>> {
    let res = db::get_guesses(id);
    if let Ok(guesses) = res {
        return Json(Resp::success(guesses))
    } else {
        return Json(Resp::err("invalid problem"))
    }
}